...
(function () {
  ...
  const option = {
    tooltip: {
      trigger: 'item',
      formatter: '{a} <br/>{b} : {c} ({d}%)' // 格式化提示信息
      // a 代表 series系列图标名称 name
      // b 代表 series数据名称 data 里面的 name
    ...
    series: [
      {
        ...
        name: '点位统计',
        type: 'pie',
        // 南丁格尔玫瑰图有两个圆（内外），第一个值是内圆的半径，第二个值是外圆的半径
        // 参数可以写像素（不带单位），也可以写百分比
        radius: ['15%', '59%'],
        // 图标的中心位置 left， top，距离图表DOM容器
        center: ['50%', '50%'],
...
(function () {
  let index = 0;
  let items = $('.order .data .item')
  let filters = $('.order .filter a')
  // console.log(items);
  items.each(function() {
    if($(this).index() === 0 || $(this).index() === 1) $(this).show();
    else $(this).hide();
  })
